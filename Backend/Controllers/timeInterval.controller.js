    ...
    try {
      ...
      if (result) {
        ...
        let newData = resData
          .filter(
            (object) =>
              object.seconds !== undefined && object.minutes !== undefined
          )
          .map((object) => ({
            key: object.keyName,
        ...
        let result = newData.reduce((acc, { key, time }) => {
          let [minutes, seconds] = time.split(":");
          // Convert minutes and seconds to integers for arithmetic operations
          minutes = parseInt(minutes, 10);
          seconds = parseInt(seconds, 10);

          // Add the seconds value to minutes
          minutes += Math.floor(seconds / 60);
          seconds %= 60;

          // Check if b (seconds) is defined before creating the result
          acc[key] = minutes + ":" + (seconds !== undefined ? seconds : 0);
          return acc;
        }, {});
        try {
          const config = {
